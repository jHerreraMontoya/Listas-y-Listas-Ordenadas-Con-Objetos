...
function List(size=5) {
    ...
    this.add = function (elem) {
        var tam = this.size(); //Declaramos una variable tamaño inicializada a 0
        var aux;
        if (this.isFull()){ //Comprobamos si la lista no esta llena
            throw FullListException(); //Si la lista esta llena lanzamos una excepción
        }
        
        if(this.isEmpty()){ //Si la lista está vacia 
            _list.unshift(elem); //Con el método unshift añadimos elementos al inicio del array y devuelve la nueva longitud
            tam++;
        }else{
            _list.push(elem); //Con el metodo push añadimos uevos elementos al array y devuelve su nueva longitud
            tam++;
...
function testList() {
    ...
    var p4 = new Person("Javier","Gutierrez");
